...
jQuery(document).ready(function ($) {
  ...
  function smoothScrollAnchors() {
    $(
      'a[href*="#"]:not([href="#"]):not([role="tab"]):not(.widget-faq-item-btn)'
    ).on("click", function () {
      if (
  ...
  function alignSubSubMenu() {
    if (
      ...
      var subSubMenu = $(
        "#header .top-header .header-navigation ul li.menu-item-has-children ul"
      );
  ...
  function newsbar() {
    ...
    animLoop(function (deltaT) {
      ...
      if (
        (rtl && currentPos >= maxTranslate) ||
        (!rtl && currentPos <= -1 * maxTranslate)
      ) {
  ...
  $(function () {
    ...
    if (scripts_data.isFront) counterNumber();

    if ($('select[name="sort-by"]').length) newsSort();
