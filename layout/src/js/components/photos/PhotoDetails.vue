          ...
          <div class="details-text">
            ...
            <div class="details-content">
              <div class="details-groups">
                <div class="details-group details-location">
                  <b>{{ $t("Landmark Location") }}:</b>
                  <div>{{ photo.location }}</div>
                </div>
                <div class="details-group details-location_to_dome">
                  <b
                    >{{
                      $t("Landmark Location relative to Dome of the rock")
                    }}:</b
                  >
                  <div>{{ photo.location_to_dome }}</div>
                </div>
                <div class="details-group details-history">
                  <b>{{ $t("Landmark History") }}:</b>
                  <div>{{ photo.history }}</div>
                </div>
                <div class="details-group details-name_reason">
                  <b>{{ $t("Reason of the name") }}:</b>
                  <div>{{ photo.name_reason }}</div>
                </div>
                <div class="details-group details-builtby">
                  <b>{{ $t("Builder Name") }}:</b>
        ...
        <div class="col-md-7 h-100">
          <div class="details-images">
            <div class="images-sticky-wrapper">
              <pics-slider
                :pic-data="{ title: photo.title, images: photo.gallery }"
              />
            </div>
          </div>
        </div>
...
export default {
  name: "PhotoDetails",
  components: { PicsSlider, DualRingLoader, ArrowIcon },
  props: {
    photo: {
      type: Object,
      default: () => null,
