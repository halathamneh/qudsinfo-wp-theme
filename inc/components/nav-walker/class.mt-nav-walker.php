...
if(!class_exists('MTL_Extended_Menu_Walker') ) {
    ...
    class MTL_Extended_Menu_Walker extends Walker_Nav_Menu
    {
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
        {
            ...
            if (!empty($children)) {
                $classes[] = 'has-sub';
                //if( wp_is_mobile() ) $value .= " style='flex:".count($children)."' ";
            }

            $class_names = join(' ', apply_filters('nav_menu_css_class', array_filter($classes), $item, $args));
            $class_names = $classes ? ' class="' . esc_attr($class_names) . '"' : '';
            ...
            $item_output .= '<i class="fa '.$fa_class.'"></i> <span>';
