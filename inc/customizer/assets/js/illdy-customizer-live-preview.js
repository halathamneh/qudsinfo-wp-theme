...
( function( $ ) {
	...
	// Show this section
	wp.customize( 'illdy_latest_info_general_show', function( value ) {
		value.bind( function( newval ) {
			if( newval == false ) {
				$( '#latest-info' ).addClass( 'customizer-display-none' );
			} else if( newval == true ) {
				$( '#latest-info' ).removeClass( 'customizer-display-none' );
			}
		} );
	} );

	// Title
	wp.customize( 'illdy_latest_info_general_title', function( value ) {
		value.bind( function( newval ) {
			$( '#latest-info.front-page-section .section-header h3' ).html( newval );
		} );
	} );

	// Entry
	wp.customize( 'illdy_latest_info_general_entry', function( value ) {
		value.bind( function( newval ) {
			$( '#latest-info.front-page-section .section-header p' ).html( newval );
		} );
	} );

	// Button Text
	wp.customize( 'illdy_latest_info_button_text', function( value ) {
		value.bind( function( newval ) {
			$( '#latest-info .latest-info-button' ).html( '<i class="fa fa-chevron-circle-right"></i>' + newval );
		} );
	} );

	// Button URL
	wp.customize( 'illdy_latest_info_button_url', function( value ) {
		value.bind( function( newval ) {
			$( '#latest-info .latest-info-button' ).attr( 'href', newval );
		} );
	} );
