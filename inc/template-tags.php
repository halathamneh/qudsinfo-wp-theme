...
if(!function_exists('illdy_archive_title')) {
	function illdy_archive_title( $before = '', $after = '' ) {
		if ( is_category() ) {
			$title = sprintf( __( '%s', 'illdy' ), single_cat_title( '', false ) );
		} elseif ( is_tag() ) {
			$title = sprintf( __( 'تاغ: %s', 'illdy' ), single_tag_title( '', false ) );
		} elseif ( is_author() ) {
			$title = sprintf( __( 'الكاتب: %s', 'illdy' ), '<span class="vcard">' . get_the_author() . '</span>' );
		} elseif ( is_year() ) {
			$title = sprintf( __( 'السنة: %s', 'illdy' ), get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'illdy' ) ) );
		} elseif ( is_month() ) {
			$title = sprintf( __( 'الشهر: %s', 'illdy' ), get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'illdy' ) ) );
		} elseif ( is_day() ) {
			$title = sprintf( __( 'اليوم: %s', 'illdy' ), get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'illdy' ) ) );
			...
			$title = sprintf( __( 'أرشيف: %s', 'illdy' ), post_type_archive_title( '', false ) );
		} elseif ( is_tax() ) {
			$tax = get_taxonomy(get_queried_object()->taxonomy);
			/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
			$title = sprintf(__('%1$s', 'illdy'), single_term_title('', false));
		} elseif ( is_search() ) {
			$title = sprintf(__('بحث عن : %1$s','illdy'),get_search_query());
		} else {
			$title = __( 'الأرشيف', 'illdy' );
		}
